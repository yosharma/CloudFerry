...
class SSHChunksTransfer(driver_transporter.DriverTransporter):
    def transfer(self, data):
        ...
        temp_dir_name_src = files.GetTempDir(runner_src, prefix).get()
        temp_dir_name_dst = files.GetTempDir(runner_dst, prefix).get()
        src_temp_dir = os.path.join(temp_dir_name_src, '')
        dst_temp_dir = os.path.join(temp_dir_name_dst, '')

        attempts_count = self.cfg.migrate.retry
        part_size = self.cfg.migrate.ssh_chunk_size

        with files.RemoteDir(runner_src, temp_dir_name_src):
